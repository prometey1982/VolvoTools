...
struct D2Messages {
  ...
  static D2Message createSetMemoryAddrMsg(ECUType ecuType, uint32_t offset);
  static D2Message createCalculateChecksumMsg(ECUType ecuType, uint32_t offset);
  static D2Message createReadOffsetMsg2(ECUType ecuType, uint32_t offset);
  static D2Message createReadDataByOffsetMsg(ECUType ecuType, uint32_t offset,
                                             uint8_t size);
  static D2Message createReadDataByAddrMsg(common::ECUType ecuType,
                                           uint32_t addr, uint8_t size);
  static std::vector<D2Message>
  createWriteDataMsgs(ECUType ecuType, const std::vector<uint8_t> &bin);
  static std::vector<D2Message>
  createWriteDataMsgs(ECUType ecuType, const std::vector<uint8_t> &bin,
                      size_t beginOffset, size_t endOffset);
  static D2Message createReadTCMDataByAddr(uint32_t addr, size_t dataSize);
  static D2Message createWriteDataByAddrMsg(ECUType ecuType, uint32_t addr,
                                            uint8_t data);
  ...
  static D2Message createJumpToMsg(ECUType ecuType, uint8_t data1 = 0,
