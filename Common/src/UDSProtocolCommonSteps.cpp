...
namespace common {
	...
	OpenChannelsStep::OpenChannelsStep(j2534::J2534& j2534, CommonStepData& commonData)
	...
	{
		_commonData.channels.emplace_back(openUDSChannel(_j2534, 500000, _commonData.canId));
		_commonData.channels.emplace_back(openLowSpeedChannel(_j2534, CAN_ID_BOTH));
		_commonData.channelIndex = getChannelIndexByEcuId(_commonData.configurationInfo, _commonData.carPlatform,
			_commonData.ecuId, _commonData.channels);
		return true;
	}

	CloseChannelsStep::CloseChannelsStep(CommonStepData& commonData)
	...
	{
		_commonData.channels.clear();
		return true;
	}

	FallingAsleepStep::FallingAsleepStep(const CommonStepData& commonData)
	...
	{
		std::vector<std::vector<unsigned long>> msgIds(_commonData.channels.size());
		for (size_t i = 0; i < _commonData.channels.size(); ++i) {
			const auto ids = _commonData.channels[i]->startPeriodicMsgs(UDSMessage(0x7DF, { 0x10, 0x02 }), 5);
			if (ids.empty()) {
				return false;
			}
			msgIds[i] = ids;
		...
		for (size_t i = 0; i < _commonData.channels.size(); ++i) {
			_commonData.channels[i]->stopPeriodicMsg(msgIds[i]);
		}
		return true;
	}

	KeepAliveStep::KeepAliveStep(const CommonStepData& commonData)
		: UDSProtocolStep{ UDSStepType::FallingAsleep, 100, true }
		, _commonData{ commonData }
	{
	}
	...
	{
		const auto& channel = *_commonData.channels[_commonData.channelIndex];
	...
	WakeUpStep::WakeUpStep(const CommonStepData& commonData)
	...
	AuthorizingStep::AuthorizingStep(const CommonStepData& commonData, const std::array<uint8_t, 5>& pin)
	...
	{
		const auto& channel = *_commonData.channels[_commonData.channelIndex];
		const auto canId{ _commonData.canId };
		channel.clearRx();
	...
	DataTransferStep::DataTransferStep(UDSStepType step, const CommonStepData& commonData, const VBF& data)
	...
	FlashErasingStep::FlashErasingStep(const CommonStepData& commonData, const VBF& flash)
	...
	StartRoutineStep::StartRoutineStep(const CommonStepData& commonData, const common::VBFHeader& header)
