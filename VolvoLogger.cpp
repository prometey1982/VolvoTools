...
bool processRegistry(const std::wstring& keyName, std::wstring& libraryPath, std::string& deviceName)
{
    ...
    if (canXonXoff > 0) {
        ...
        deviceName = toString(key->GetString(L"Name"));
        return false;
    }
    return true;
}

std::pair<std::wstring, std::string> getLibraryParams()
{
...
bool getRunOptions(int argc, const char* argv[], unsigned long& baudrate, std::string& paramsFilePath, std::string& outputPath)
{
    using namespace boost::program_options;
    options_description descr;
    descr.add_options()
        ("baudrate,b", value<unsigned long>()->default_value(500000), "CAN bus speed")
        ("variables,v", value<std::string>()->required(), "Path to memory variables")
