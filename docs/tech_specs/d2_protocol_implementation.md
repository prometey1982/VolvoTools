...
[3] = requestId[0]              (эхо номера сервиса, как в обычном ответе)
[4] = код ошибки → выбрасывается D2Error
```

Регион ошибки — 3 байта потока данных `[ecuId, 0x7F, requestId[0]]` (формат уточнён по железу, см. `d2_request_parser_hardening.md` Шаг 7: полного эха requestId в реальном ответе нет — эхо только номера сервиса). Как только регион собран, разбор бросает `D2Error` с кодом = `result[3]` (код ошибки ЭБУ сразу после эха номера сервиса); если байт кода отсутствует → `D2Error(0)`. Throw происходит сразу после первого фрейма, без ожидания конца серии: не зависит от длины requestId (при длинном requestId старый разбор уходил в таймаут), guard'ы `result.size() >= 3` / `> 3` исключают OOB на коротких кадрах.
...
| Тест | Описание |
|------|----------|
| `ErrorResponse` | `[1]=ecuId, [2]=0x7F, [3]=requestId[0], [4]=код` → `D2Error` |
| `ErrorWrongEcuId` | `[1] != ecuId` → не ошибка, continue |
| `ErrorNo7F` | `[2] != 0x7F` → не ошибка |
| `ErrorResponseLongRequestId` | Ошибка при длинном requestId — эхо только номера сервиса, регион 3 байта → `D2Error` |
